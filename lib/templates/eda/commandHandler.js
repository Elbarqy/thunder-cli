...
const generateCommandHandler = name => {
    ...
    import {Create${name}Command, Read${name}Command, Remove${name}Command,Update${name}Command} from '../../commands/${name}/${name}.command'
    `;
	let handlers = [];
	CRUD.map(event => {
		const handlerName = `${event}${name[0].toUpperCase()}`;
		handlers.push(`
